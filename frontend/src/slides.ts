...
function buildInterface(defaults: SlideSettings, preferences: StoredPreferences): HTMLElement {
  ...
  root.innerHTML = `
    <div class="slides-toast" role="status" aria-live="polite"></div><canvas class="slides-ink" aria-label="Drawing canvas"></canvas><div class="slides-laser" aria-hidden="true"></div><div class="slides-edge-scrollbar" role="scrollbar" aria-label="Slide scroll position" aria-orientation="vertical" aria-valuemin="0" aria-valuemax="0" aria-valuenow="0" tabindex="0" hidden><span class="slides-edge-scrollbar-thumb"></span></div>
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  void deck.initialize().then(() => {
    ...
    deck.on("slidechanged", () => { updateNavigation(); scheduleSlideChrome(); if (multiNote) void hydrateWindow(noteIndex()).then(() => { buildNavigation(); updateNavigation(); scheduleApply(); }).catch(() => say("This file could not be loaded. Select it again to retry.")); }); window.addEventListener("resize", scheduleApply); revealElement.querySelectorAll("img,video,iframe").forEach((media) => media.addEventListener("load", scheduleApply)); window.setTimeout(scheduleApply, 500); document.addEventListener("pointermove", wake, { passive: true }); document.addEventListener("keydown", (event) => { if (event.key === "Escape") { if (activePanel) panelClose(); else if (tool) setTool(null); return; } if (event.target instanceof HTMLInputElement || event.target instanceof HTMLSelectElement) return; if (event.key === "?") panelOpen("help", dock.querySelector("[data-action=help]")!); else if (event.key.toLowerCase() === "g") panelOpen("picker", dock.querySelector("[data-action=picker]")!); else if (event.key.toLowerCase() === "f") { if (document.fullscreenElement) void document.exitFullscreen(); else void document.documentElement.requestFullscreen?.().catch(() => say("Fullscreen is unavailable")); } else if (event.key.toLowerCase() === "l") setTool("laser"); else if (event.key.toLowerCase() === "z") setTool("magnify"); else if (event.key.toLowerCase() === "d") setTool("pen"); else if (event.key.toLowerCase() === "b") deck.togglePause(); });
  });
});
