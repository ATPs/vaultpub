...
While presenting, move the pointer to reveal the compact presenter dock at the bottom centre. It appears faintly until it is hovered or focused, then becomes clear enough for use. Every dock button has a hover/focus explanation. Its gear opens a wider, compact popup with reading-theme colour swatches, transition, text size from 75% to 300%, aspect ratio, code wrapping, split, centering, progress, slide-number, and reduced-motion preferences. A popup remains open until its close button, an outside click/tap, or Escape. Viewer preferences are remembered in the browser and **Reset to deck defaults** restores the note's frontmatter choices. Code wrapping is on by default.
