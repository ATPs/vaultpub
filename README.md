...
While presenting, move the pointer to reveal the compact presenter dock at the bottom centre. It appears faintly until it is hovered or focused, then becomes clear enough for use. Every dock button has a hover/focus explanation. Its gear opens a compact grouped popup with reading-theme colour swatches, transition, text size from 75% to 300%, aspect ratio, code wrapping, split, centering, progress, slide-number, and reduced-motion preferences. Split choices explain their behavior on hover or keyboard focus. A popup remains open until its close button, an outside click/tap, or Escape. Viewer preferences are remembered in the browser and **Reset to deck defaults** restores the note's frontmatter choices. Code wrapping is on by default.
...
Split choices are `auto`, `chapters`, `sections`, `detail`, `fit`, `explicit`, and `single` (legacy `h1`, `h2`, and `h3` URLs remain accepted). `auto` keeps the normal behavior: standalone `---` first, then H2 headings, then one scrollable slide. Chapters uses H1, Sections H2, and Detail H2/H3. **Fit viewport** keeps explicit source boundaries and repaginates rendered content whenever the aspect ratio, viewport, theme, text size, wrapping, or media layout changes. It keeps the selected text size, allowing pages to grow in number rather than silently shrinking a 300% setting back to normal. Lists, code blocks, tables, callouts, and consecutive images are packed into readable continuation pages; a table stays whole when it fits and otherwise continues with multiple rows per page. Images remain present when pages are rebuilt. Fit pages do not use ordinary vertical scrolling for content that can be paginated. `single` creates a flush single-note reading surface: Reveal has no surrounding margin, the page scrollbar reaches the viewport edge, and its thumb stays subtle until hovered. It always hides Reveal's corner slide number while retaining the presenter dock. Other Slide View layouts use the same thin main scrollbar at the browser edge, while their optional page number is plain text in the visible slide's lower-right corner. A shareable `?split=sections` URL temporarily overrides all defaults. A saved browser split preference applies after that, followed by `slide.split` frontmatter and `auto`.
